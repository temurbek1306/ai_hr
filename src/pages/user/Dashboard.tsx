...
import { Book, FileText, Target, BarChart, Bell, Loader2 } from 'lucide-react'
...
export default function UserDashboard() {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                setIsLoading(true);
                // Fetch current user profile and summary
                const [profile, summary] = await Promise.all([
                    profileService.getProfile(),
                    profileService.getSummary()
                ]);
                setUserData({ ...profile.body, ...summary.body });
                setError(null);
            } catch (error: any) {
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-display font-bold text-gray-900">
                            Xush kelibsiz, {userData?.firstName || 'Foydalanuvchi'}!
                        </h1>
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <h2 className="text-xl font-semibold mb-2">
                                    Jarayon: {userData?.currentStatus || 'Onboarding'}
                                </h2>
                                <p className="text-blue-100 mb-4 max-w-md">
                                    {userData?.statusDescription || "Siz hozir sinov muddatidasiz. Muvaffaqiyatli o'tish uchun belgilangan testlarni topshiring."}
                                </p>
